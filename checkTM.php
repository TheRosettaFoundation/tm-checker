...
20-04-2012
Added XLIFF and tab-delimited TXT file handling.
*/
...
function checkTM_TMX($file){
	...
	foreach ($tus as $tu) {
		...
		foreach ($tuvs as $tuv) {
			...
			if($lang == '') {	//seems to be non-standard!
	...
	//Call scan_array() to perform totalling of duplicates, cat2, cat3 and number of entries
	$results = 'File type: TMX <br> '.scan_array($TM_array);
...
function checkTM_XLIFF($file){
	...
	//Call scan_array() to perform totalling of duplicates, cat2, cat3 and number of entries
	$results = 'File type: XLIFF <br> '.scan_array($TM_array);
...
function checkTM_TXT($file){
	...
	// Fill $TM_array[][] array with source/target pairs
	$a = $file;
	...
	//Call scan_array() to perform totalling of duplicates, cat2, cat3 and number of entries
	$results = 'File type: TXT <br> '.scan_array($TM_array);
